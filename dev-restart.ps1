...
# 作用: 自动检测并杀掉旧的 WebOps 前后端进程 -> 重新启动前后端
...
$BACKEND_PORT = 8001
$FRONTEND_PORT = 5174
$PYTHON = "D:\programming\environment\miniforge3\envs\webops\python.exe"
$PROJECT_ROOT = "D:\programming\code\WebOps"
$FRONTEND_DIR = "D:\programming\code\WebOps\webops\frontend"

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  WebOps 一键启动 (后端 8001 / 前端 5174)" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan

# ---------- 1. 杀掉旧的 WebOps 后端进程（含 uvicorn / spawn_main worker） ----------
Write-Host "`n[1/5] 检测并停止旧后端进程..." -ForegroundColor Yellow
$backendProcs = Get-CimInstance Win32_Process | Where-Object {
    $_.CommandLine -match 'webops\.server\.main' -or
    ($_.CommandLine -match 'spawn_main' -and $_.CommandLine -match 'webops')
}
if ($backendProcs) {
...
$frontProcs = Get-CimInstance Win32_Process | Where-Object {
    ...
    ($_.CommandLine -match 'webops\\frontend') -or
...
# 检查 LLM 密钥：优先环境变量，其次 webops.config.json 的 api_key
if (-not $env:WEB_OPS_LLM_API_KEY) {
    try {
        $cfg = Get-Content "$PROJECT_ROOT\webops.config.json" -Raw | ConvertFrom-Json
        if ($cfg.llm.api_key) {
            $env:WEB_OPS_LLM_API_KEY = $cfg.llm.api_key
            Write-Host "  - 已从 webops.config.json 读取 api_key"
        }
    } catch { }
}
if ($env:WEB_OPS_LLM_API_KEY) {
    Write-Host "  - LLM 密钥已就绪（可执行真实行为树）"
} else {
    Write-Host "  - 警告: 未配置 LLM 密钥。执行行为树将报『未配置 LLM API 密钥』。" -ForegroundColor Yellow
    Write-Host "    解决: 在 webops.config.json 填入 llm.api_key（内部工具，简单优先），或先执行 \$env:WEB_OPS_LLM_API_KEY=\"sk-...\" 再运行本脚本。" -ForegroundColor Yellow
}
Start-Process -FilePath $PYTHON -ArgumentList "-m uvicorn webops.server.main:app --host 127.0.0.1 --port $BACKEND_PORT" -WorkingDirectory $PROJECT_ROOT -WindowStyle Normal
...
Write-Host "  (端口已避开 NexusOps: 本机 8000/5173 为 NexusOps 使用，WebOps 用 8001/5174)" -ForegroundColor Yellow
Write-Host "========================================" -ForegroundColor Cyan