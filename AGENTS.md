...
用户书写**行为树文档**（yaml/dict，结构确定），AutoBranch 用 LLM 理解页面、在叶子节点（Action/Condition）内执行有界代理动作，输出操作日志与页面截图。核心设计：**流程流转确定，LLM 只在单节点内行使有限执行权**。详细契约见 `docs/contract.md`。

- **引擎层**（Python）：M0 LLM 客户端 / M1 行为树解析器（含 FunctionCall）/ M2 变量空间（泛型 object）/ M3 插件框架 / M4 叶子 agent / M5 编排器 / M6 报告
- **插件集**（M7，`autobranch/plugins/`）：浏览器（自包含驱动+语义图）/ 计算 / SSH / 文件 + 用户自定义插件（DB 源码，仅标准库）
- **管理系统**：M8 后端（FastAPI，含插件管理 API）+ M9 前端（React + TypeScript + Vite，含插件管理页）

模块 spec 见 `docs/specs/`（M0~M9b 各一份）。

## 开发与协作约束

1. **独立的 conda 环境**：本机已安装 conda，项目开发与测试验证必须创建**独立的 conda 环境**（如 `conda create -n autobranch python=3.x`），禁止使用本机已有环境。依赖安装、测试、运行一律在该环境内执行。
...
3. **测试与实机验收**：每次修改代码都必须设计对应的测试用例，特别是**集成化测试**。项目含前后端时，需**实际运行前端与后端进行功能测试**，不能仅凭单元测试通过验收。
   - **端到端测试（E2E）必做**：任何涉及管理系统的改动，必须包含**通过模拟前端操作**的端到端测试（Playwright 驱动真实浏览器操作 UI：点击/跳转/断言页面），**不得只用后端 API 调用或单元测试代替**。E2E 覆盖"前端执行行为树 → 轮询渲染 → 报告展示"核心用户链路（`autobranch/frontend/e2e/`，`npm run test:e2e`）。
...
# 后端引擎（Python，在 autobranch 环境内）
pip install -e .
pytest                                        # 引擎单元测试
pytest -m integration                         # 集成测试（真实浏览器）

# 管理系统后端（FastAPI，在 autobranch 环境内；注意禁用 8000，用 8001）
uvicorn autobranch.server.main:app --reload --port 8001   # 本地开发
