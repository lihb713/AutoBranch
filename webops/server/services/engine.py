...
from webops.reporting.models import NodeReport
...
class EmbeddedEngineService(EngineService):
    ...
    :param config: 引擎统一配置（``WebOpsConfig``，M0/M5/M7 接线）。
    ...
    def _config_error_result(self, run_id: int) -> RunResult:
        ...
        reason = (
            "未配置 LLM API 密钥：请在 webops.config.json 填入 api_key，"
            "或设置环境变量 WEB_OPS_LLM_API_KEY 后重启后端"
        )
    ...
    def _build_run_config(self) -> RunConfig:
        ...
        return cfg.to_run_config(
            ...
            engine=getattr(self, "_last_functions", None),
        )

    def _default_engine(self, run_id: int, report_root: Path) -> Engine:
        """默认装配（参照 tests/e2e/run_e2e.py）：真实浏览器 + 真实引擎函数。"""
        ...
        engine = Engine(
            ...
            reporter_factory=build_reporter_factory(
                run_id, report_root, default_screenshotter(browser)
            ),
        )
    ...
    @property
    def last_functions(self) -> EngineFunctions | None:
        """最近装配的 M5 引擎函数（供测试断言注入）。"""
        return getattr(self, "_last_functions", None)
