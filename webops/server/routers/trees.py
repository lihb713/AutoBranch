...
from webops.server.schemas.check import CheckReportOut
from webops.server.schemas.run import RunStartOut
...
def check_tree(tree_id: int, service: TreeServiceDep):
    ...
    return CheckReportBuilder.build(tree.content, tree.name)
...
def run_tree(
    ...
    tree = service.get(tree_id)
    validate_document(tree.content, tree.name)
