"""WebOps 统一配置加载（工具侧配置，非用户行为树文档）。

集中管理程序运行参数：LLM 配置（base_url/api_key/model）、浏览器配置
（类型/无头/超时）、运行参数（叶子超时/轮数上限/报告目录等）。配置来源：

1. 默认值（代码内）
2. 配置文件（JSON，路径由 ``--config`` 或环境变量 ``WEB_OPS_CONFIG`` 指定）
3. 环境变量覆盖（当前仅 ``WEB_OPS_LLM_API_KEY`` 用于密钥安全注入）

**api_key 安全**：api_key 属于敏感信息，配置文件中的 ``api_key`` 可为空，
加载时优先取环境变量 ``WEB_OPS_LLM_API_KEY``（不把密钥写入配置文件/仓库）。
"""
...
#: 默认配置文件路径（项目根）。
DEFAULT_CONFIG_PATH = "webops.config.json"
#: 运行目录（报告/截图等产物的默认根目录）。
...
@dataclass(frozen=True)
class WebOpsConfig:
    """WebOps 统一配置（配置文件的根结构）。

    JSON 结构示例（``webops.config.json``）：
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> WebOpsConfig:
    ...
    @classmethod
    def load(cls, path: str | Path | None = None, *, apply_env: bool = True) -> WebOpsConfig:
        """从配置文件加载；缺失配置项回落默认值。

        :param path: 配置文件路径；None 依次尝试 ``WEB_OPS_CONFIG`` 环境变量与
          项目根 ``webops.config.json``（不存在则用全默认）。
        :param apply_env: 是否应用环境变量覆盖（``WEB_OPS_LLM_API_KEY``）。
        """
    ...
    def to_llm_config(self) -> Any:
        ...
        from webops.llm import LLMConfig
    ...
    def to_browser_config(self) -> Any:
        """构建 M1 ``BrowserConfig``。"""
        from webops.browser import BrowserConfig
...
def _resolve_path(path: str | Path | None) -> str | None:
    if path is not None:
        return str(path)
    env_path = os.environ.get("WEB_OPS_CONFIG")
    if env_path:
        return env_path
...
def _apply_env_overrides(cfg: WebOpsConfig) -> WebOpsConfig:
    """环境变量覆盖：``WEB_OPS_LLM_API_KEY`` 优先于配置文件（安全注入密钥）。"""
    key = os.environ.get("WEB_OPS_LLM_API_KEY")
    if key:
        return WebOpsConfig(
            llm=LLMOptions(
