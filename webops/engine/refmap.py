...
from webops.browser import DomSnapshot, ElementNode
from webops.semantic_graph import Element, SemanticGraph
...
class EngineRefMap:
    ...
    def refresh(self, graph: SemanticGraph, snapshot: DomSnapshot | None = None) -> None:
        ...
        nodes, parents = self._index_nodes(snapshot)
        ...
        for element in graph.elements:
            ...
            entries[element.ref] = RefResolution(
                ...
                selector=self._selector_for(element, node, nodes, parents),
                tag=element.tag,
                role=element.role,
    ...
    def _selector_for(
        ...
        tags: list[str] = []
        seen: set[str] = set()
        current: ElementNode | None = node
        while current is not None and current.id != "root" and current.id not in seen:
            seen.add(current.id)
            tags.append(current.tag)
            target_depth = current.depth - 1
            parent_id = parents.get(current.id)
            next_node: ElementNode | None = None
            while parent_id is not None and parent_id != "root":
                parent = nodes[parent_id]
                if parent.depth == target_depth:
                    next_node = parent
                    break
                parent_id = parents.get(parent_id)
            current = next_node
        return " > ".join(["body", *reversed(tags)])
