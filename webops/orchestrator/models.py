...
from webops.schema.models import Value

if TYPE_CHECKING:
    from webops.browser.config import BrowserConfig
    from webops.engine import EngineFunctions
    from webops.llm import LLMConfig, LLMSession, ToolSpec
    from webops.reporting.models import ExecReport, TraceReport
...
class RunConfig:
    ...
    :param engine: M5 引擎函数（真实叶子执行需要；mock 叶子无需）。
    :param session_factory: 会话工厂（传 M6，测试注入假传输用）。
    :param tools: 工具注册表（默认 M5 ``ENGINE_TOOLS``）。
    """
    ...
    engine: EngineFunctions | None = None
