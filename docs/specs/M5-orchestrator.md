...
>
> **实现状态：已实现 ✅**（OpenSpec change `m7-orchestrator`，2026-08）。代码位于 `autobranch/orchestrator/`，测试位于 `tests/orchestrator/`（含 `tests/orchestrator_helpers.py` 共享 mock 构造器）。本文件已与实现同步；涉及契约语义的实现细节见「与契约的接口细节」一节，供统一更新 `docs/contract.md`。
...
- **schema 命名空间**（来自 M3）：帧管理（`enter_frame`/`exit_frame`）、配置继承（`resolve_config`）、页面变量（仅显式 `save_to` 保存，如 `NewParam.名:page_ref`）
...
- **浏览器会话生命周期管理**：浏览器插件**懒装配冷启动**（`init(runtime)` 时启动驱动，纯计算行为树零浏览器开销）；运行结束 `release` 统一释放
...
class RunConfig:
    ...
    initial_graph_scope: str = "full"  # （兼容保留，传 M6）
    initial_graph_lod: int = 2         # （兼容保留，传 M6）
    report_dir: str = "reports"        # 报告持久化根目录（M6）
    browser_config: BrowserConfig | None = None
    global_config: dict[str, Value] = {}   # 额外全局默认配置（注入根级 schema）
    llm_config: LLMConfig | None = None    # 真实叶子执行需要（M0）
    session_factory: ...                   # 传 M6（可选，测试注入假传输）
```

**真实叶子执行的接线说明**：`run` 默认经 `make_default_leaf_executor(config, space, registry=..., runtime=...)`
构造 `LeafContext`（含生效 timeout）并调用 M6 `execute_leaf`；需要 `config.llm_config`（M0）
与共享插件注册表（M3），否则抛出 `OrchestratorError`。测试全部注入 mock 叶子执行器
（`(node, timeout) -> LeafResult`），不依赖 M0/M3/M6 真实实现。
...
- 测试数：81 个（`tests/orchestrator/`，静态统计 `def test_`，参数化后实际收集更多）；
  全库 762 个测试函数（静态统计），全部经 `autobranch` conda 环境 `pytest` 通过；
  `ruff check .` 无告警。
...
## 8. FunctionCall 节点执行与插件分发（能力插件化新增，已实现）

### 8.1 FunctionCall 节点执行

遍历到 FunctionCall 节点时**确定性调用**（不经 LLM）：按 `function` 经插件框架分发，实参从当前帧求值（元素与 ref `args` 同构：本树裸变量名或字面量），返回值按 `returns` 回收写父帧（按声明类型）。函数不存在 / 实参求值失败（变量未定义）/ 执行失败 → 节点 FAILURE，失败原因含函数名与错误。

### 8.2 插件分发与懒装配

经插件框架统一分发：按函数名定位插件 → 未装配则懒装配（`init`；重资源如浏览器 run 内首次用到才启动）→ 调用 → 产出型工具由引擎落笔写变量。**运行结束（含失败/异常提前终止）统一 `release` 重资源**（finally 保证）。

### 8.3 截图

插件模式叶子不无条件截图；截图由浏览器插件 `semantic_graph` 产出，经 `LeafResult.screenshots` 写入 `NodeReport.screenshot_path`（报告展示）。
