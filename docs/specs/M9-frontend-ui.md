...
- **画布呈现**：每个节点为固定尺寸卡片，类型用颜色/形状/图标区分，显示用户自定义 `name`；树从根节点在上、向下生长，兄弟水平排布，父子以连线绘制；节点位置自适应布局，不需手动摆放。画布分**主树区**（`root` 沿槽位引用递归可达）与**游离区**（与 Root 不联通的游离树，顶部带根 name + 「游离」标记）。**画布为内部滚动视口**（宽/高不再撑开页面、不挤占右侧属性面板），并提供**缩放控制条**（缩小/百分比/放大/适应/100%）。
...
- **文档接口编辑**：**行为树级属性**，与树名同区展示于编辑器工具栏下方（`DocInterfaceEditor`，不在节点属性面板内）——编辑树 `inputs`（名→类型，类型 str/int/float/bool/page_ref/object）与 `outputs`（名列表），可增删改；ref 参数对齐依据该声明。只读模式（ref 预览）下禁用。
