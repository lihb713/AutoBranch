...
WebOps 的后端服务：行为树文档 CRUD API、清晰度校验（复用 M2）、执行触发与状态查询（内嵌引擎 M7）、报告/截图存储与提供。**引擎作为 Python 库内嵌于同一进程（§12.3），无进程间通信。依赖 M2 + M7 + M8。**

实现位置：`webops/server/`（FastAPI 应用、routers、schemas、services、models、db）。
...
| 清晰度校验 | 保存时校验（复用 M2 解析器），执行前再校验 | §12.5/§4.4 | ✅ `services/validation.py`（单一入口，两处复用） |
...
- **持久化数据**：文档库（SQLite `data/webops.db`）、报告文件（`data/reports/`）
...
- **实机验收**：`uvicorn webops.server.main:app --reload` 启动，HTTP 请求验证 health/CRUD/check/run/state/report/trace/reports 全部可用。

## 8. 运行方式

```bash
conda run -n webops --no-capture-output python -m uvicorn webops.server.main:app --reload
# 可选环境变量：WEBOPS_DB_PATH（默认 data/webops.db）、WEBOPS_REPORT_DIR（默认 data/reports）
# 引擎真实执行需配置 api_key：WEB_OPS_LLM_API_KEY=sk-...（契约 §6.3）
```