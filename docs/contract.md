...
```
主流程(T)  schema: T/
  ├── 登录(A)  schema: T/登录/        ← T 的直接子, T 可见
  │     └── 输入框(A')  schema: T/登录/输入框/   ← T 不可见
  └── 导出(B)  schema: T/导出/        ← T 的直接子, T 可见

T 可读/写:  T/xxx, T/登录/xxx, T/导出/xxx
T 不可读写: T/登录/输入框/xxx         ← 不知道 A 内部结构
```
...
```
T 写 T/登录/username      ← T 只负责给直接子传参
登录写 T/登录/输入框/值    ← 登录负责给它自己的直接子传参
```

**取返回值 = 读直接子 schema（对称）：**
```
导出块 B 完成后，把结果写到 T/导出/result
T 读取 T/导出/result      ← 获取子块返回值的唯一途径
```
...
可见性 (严格):
  每块只读写: 自己的 schema + 直接子块的 schema
  不访问:     祖先 / 兄弟 / 孙子 的 schema

传参逐层传递:  T → 登录 → 登录的子块 ... (每层只处理直接子)
...
```
功能:
  帧模型: 块实例的独立命名空间 (§5.7.4)
  严格作用域: 读/写自己 + 直接子块 (§5.3.2)
  配置参数继承: 向上查找 (§5.3.4)
  页面变量机制 (§5.10)
