...
每次创建变量时声明预期类型（金额 / 订单号 / URL / 日期 / **页面引用**...），规则引擎据此强校验，提取错误则断言失败并终止。

**支持的类型集合**（M3 已落地）：

```
SUPPORTED_TYPES = [金额, 订单号, URL, 日期, 文本, 页面引用, 布尔, 整数, 数字]
- "金额" 接受纯数字字符串（拒绝含货币符号文本 / 布尔）
- "整数"/"数字" 用于配置参数（timeout/retry）的校验
- 页面引用 = PageRef 类型变量（§5.10）
```

```
示例:
  Step:
    action: 提取"订单金额" {{set:this/amount}}
    expect: {{get:this/amount}} 是数字 且 在 0~100000 之间

页面引用类型 (见 §5.10):
  $this/登录页 = open("https://.../login")   ← 类型: 页面引用
...
1. 打开并存页签: open(url, save_to) — 打开 url 新建页签，把页面引用写入 save_to
   （如 {{set:page:this/页面A}} 声明存页签时）；save_to 省略写默认活动页变量
2. 切回已存页签: activate(page_var) — 把已存页面变量指向的页签设为当前活动页
   （只切焦点，不新建）；描述如"切回/使用已打开的 X 页"时调用
3. 取 URL 字符串: get_url(save_to) — 存当前活动页 url 为文本（{{set:string:...}}）
...
**类型化 set 语法**：`{{set:page:变量}}` = 存页签引用（blackboard 存 PageRef）；
`{{set:string:变量}}` = 存 url 文本。**标注即类型契约**——open 产物恒为 PageRef、
...
**LLM 视角**：LLM 每次只面对**当前活动页**的语义图（一页），跨页数据走变量、不跨页记忆。多标签页并存由 `{{set:page:...}}` 命名的页面变量承载；**切换由行为树描述显式表达**（"切回 X 页"→ activate、新开/访问 → open），LLM 按描述选函数，引擎保证活动页切换确定性。
