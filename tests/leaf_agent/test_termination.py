...
def test_round_limit_terminates_leaf(config, fake, stub_engine):
    ...
    fake.responses = [
        chat_response(
            tool_calls=[tool_call("browser.semantic_graph", {"scope": "full", "lod": i})]
        )
        for i in range(11)
    ]
    stub_engine.results["browser.semantic_graph"] = graph_result("PAGE: 测试  URL=x")
...
def test_no_progress_terminates_leaf(config, fake, stub_engine):
    ...
    call = tool_call("browser.semantic_graph", {"scope": "full", "lod": 2})
...
def test_location_termination_is_covered_by_leaf_termination(config, fake, stub_engine):
    ...
    call = tool_call("browser.semantic_graph", {"scope": "F1", "lod": 3})
    fake.responses = [chat_response(tool_calls=[call]), chat_response(tool_calls=[call])]
