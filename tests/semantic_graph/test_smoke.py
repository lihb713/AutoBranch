...
默认跳过；设置环境变量 ``WEB_OPS_TEST_LLM=1`` 且提供 ``WEB_OPS_LLM_BASE_URL`` /
``WEB_OPS_LLM_API_KEY`` / ``WEB_OPS_LLM_MODEL`` 时运行，人工检查 purpose/related-to
合理性（§8.5 真实 LLM 填充）。
"""
...
from webops.semantic_graph import LLMSessionFiller, semantic_graph, serialize

pytestmark = pytest.mark.smoke

_ENABLED = os.environ.get("WEB_OPS_TEST_LLM") == "1"


@pytest.mark.skipif(not _ENABLED, reason="真实 LLM 冒烟：需 WEB_OPS_TEST_LLM=1 与环境配置")
def test_real_llm_fill(sg_driver, sg_http_server, sg_open_page):
    base_url = os.environ.get("WEB_OPS_LLM_BASE_URL", "https://opencode.ai/zen/go/v1")
    api_key = os.environ["WEB_OPS_LLM_API_KEY"]
    model = os.environ.get("WEB_OPS_LLM_MODEL", "deepseek-v4-flash")

    page_ref = sg_open_page(f"{sg_http_server}/sg_login.html")
    session = LLMSession(
        LLMConfig(base_url=base_url, api_key=api_key, model=model),
        system_prompt="你是 WebOps 页面语义分析器，输出合法 JSON。",
    )
