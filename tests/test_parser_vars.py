...
def test_action_set_targets_extracted():
    ...
    result = parse_doc("主", {"block 主": {"Action": "提取金额 [[set:this/amount]]"}}, None)
...
def test_get_out_of_scope_fails():
    ...
    doc = {
        "block 主": {"Sequence": [{"ref": "this/甲"}]},
        "block 甲": {"Sequence": [{"Action": "读 [[get:this/兄弟/值]]"}]},
        "block 兄弟": {"Sequence": [{"Action": "动作"}]},
    }
    result = parse_doc("主", doc, None)
...
def test_set_out_of_scope_fails():
    ...
    doc = {
        "block 主": {"Sequence": [{"ref": "this/甲"}]},
        "block 甲": {"Sequence": [{"Action": "存 [[set:this/兄弟/值]]"}]},
        "block 兄弟": {"Sequence": [{"Action": "动作"}]},
    }
    result = parse_doc("主", doc, None)
    issues = [i for i in result.checks.issues if i.code == "scope.out_of_scope"]
...
def test_set_type_annotation_page():
    ...
    result = parse_doc(
        "主",
        {"block 主": {"Action": "打开登录页 [[set:page_ref:页面A]]"}},
        None,
    )
    assert result.checks.ok, result.checks.issues
    action = result.tree.root
...
def test_set_type_annotation_string():
    ...
    result = parse_doc(
        "主",
        {"block 主": {"Action": "记下地址 [[set:str:url]]"}},
        None,
    )
    assert result.checks.ok, result.checks.issues
...
def test_set_type_annotation_with_this_prefix():
    ...
    result = parse_doc(
        "主",
        {"block 主": {"Action": "开 [[set:page_ref:this/页面B]]"}},
        None,
    )
    assert result.checks.ok, result.checks.issues
...
def test_set_no_type_default_empty():
    ...
    result = parse_doc(
        "主",
        {"block 主": {"Action": "提取 [[set:this/amount]]"}},
        None,
    )
    assert result.checks.ok, result.checks.issues
