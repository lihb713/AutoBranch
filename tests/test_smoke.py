...
环境变量：
- ``WEB_OPS_LLM_BASE_URL``：OpenAI 兼容端点（默认 https://api.deepseek.com）
- ``WEB_OPS_LLM_API_KEY``：API 密钥（缺失则跳过）
- ``WEB_OPS_LLM_MODEL``：模型名（默认 deepseek-chat）
"""
...
from webops.llm.models import ToolSpec
from webops.llm.session import LLMSession

pytestmark = pytest.mark.smoke

BASE_URL = os.environ.get("WEB_OPS_LLM_BASE_URL", "https://api.deepseek.com")
API_KEY = os.environ.get("WEB_OPS_LLM_API_KEY", "")
MODEL = os.environ.get("WEB_OPS_LLM_MODEL", "deepseek-chat")


def test_smoke_real_endpoint():
    """真实请求冒烟：需设置 WEB_OPS_LLM_API_KEY 才运行。"""
    if not API_KEY:
        pytest.skip("未配置 WEB_OPS_LLM_API_KEY，跳过冒烟测试")
